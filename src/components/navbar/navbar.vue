  ...
  import {commonMixin} from "@/mixins";
    ...
    mixins: [commonMixin],
		...
		watch: {
			...
			// },
      'this.$store.state.address': (newVal) => {
        console.log(`watch address`, newVal)
      }
		},
		created() {
		},
		computed: {
      ...mapGetters(['contracts', 'isConnected', 'address', 'chainId', 'walletLinkName', 'walletLink']),
			currentLang() {
      ...
      //     console.log(`mychainId`,this.mychainId)
			// 		if (this.isauto) {
			// 			if (this.mychainId === 1) {
      //         this.chainId = 0;
